...
export default function UpdateExitOrder() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [orderRes, vRes, dRes, cRes] = await Promise.all([
          axios.get(`${apiUrl}/exit-orders/exit-orders/${orderId}`, { headers: { Authorization: `Bearer ${token}` } }),
        ...
        setForm({
          vehicle_id: o.vehicle_id.toString(),
          driver_id: o.driver_id.toString(),
          client_id: o.client_id.toString(),
          exit_date: o.exit_date ? new Date(o.exit_date) : null,
          entry_date: o.entry_date ? new Date(o.entry_date) : null,
        ...
        const foundVehicle = vRes.data.find((v: Vehicle) => v.id === o.vehicle_id);
        setSearchPlaca(foundVehicle?.placa || '');
        setSelectedVehicle(foundVehicle || null);

        const foundDriver = dRes.data.find((d: Driver) => d.id === o.driver_id);
        setSearchLicense(foundDriver?.driver_control_number || '');
        setSelectedDriver(foundDriver || null);

        setSearchClient(cRes.data.find((c: Client) => c.id === o.client_id)?.client_name || '');

        if (o.exit_date && o.entry_date) {
          handleDateDiff(new Date(o.exit_date), new Date(o.entry_date));
        }

      } catch (err) {
        console.error('Error al cargar datos:', err);
        toast.error('No se pudo cargar la orden de salida.');
      }
    };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!form.vehicle_id || !form.driver_id || !form.client_id || !form.exit_date || !form.entry_date || !form.exit_time || !form.entry_time) {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      await axios.put(`${apiUrl}/exit-orders/exit-orders/update/${orderId}`, {
        ...form,
      ...
      setTimeout(() => navigate('/exit-orders'), 1500);
    } catch (err) {
  ...
  return (
    <>
      <ComponentCard title="Actualizar Orden de Salida">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Conductor */}
          <div className="relative">
            <Label className="dark:text-gray-300">Numero de Licencia</Label>
            <input
              type="text"
         ...
         <div className="flex gap-4">
          ...
          <Button variant="primary" size="md" className="w-full" disabled={loading}>
            {loading ? 'Guardando...' : 'Actualizar Orden de Salida'}
          </Button>
        </form>
      </ComponentCard>
