...
export default function WorkshopReportTable() {
  const [records, setRecords] = useState<WorkshopReport[]>([]);
  const [search, setSearch] = useState("");
  const [detalle, setDetalle] = useState<WorkshopReport | null>(null);
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        const res = await axios.get(`${apiUrl}/workshop-reports/workshop-reports/all`, {
  ...
  const filtered = useMemo(() => {
    return records.filter((r) => {
      const matchSearch =
        ...
        r.report_time.includes(search) ||
        r.report_details.toLowerCase().includes(search.toLowerCase()) ||
        r.report_part_details.toLowerCase().includes(search.toLowerCase());
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`${apiUrl}/workshop-reports/workshop-reports/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div className="overflow-hidden w-full max-w-screen-xl mx-auto rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
      <div className="p-4 flex flex-wrap gap-4 items-center">
        <input
          type="text"
          placeholder="Buscar por placa, marca, modelo, fecha, hora o detalles"
          value={search}
        ...
        <div className="ml-auto">
          <Button variant="primary" onClick={() => navigate("/create-workshop-report")}>
            Crear reporte
          </Button>
        </div>
      </div>
          ...
          <tbody>
            {filtered.map((r) => (
              ...
              >
                ...
                <td className="px-5 py-3 text-sm text-gray-700 dark:text-gray-200">{formatDate(r.report_date)}</td>
                <td className="px-5 py-3 text-sm text-gray-700 dark:text-gray-200">{r.report_time}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="block md:hidden p-4 space-y-4">
        {filtered.map((r) => (
          ...
          >
            ...
            <p><strong>Fecha:</strong> {formatDate(r.report_date)}</p>
            <p><strong>Hora:</strong> {r.report_time}</p>
          </div>
        ))}
      </div>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm text-gray-800 dark:text-gray-200">
              ...
              <div>
                <p><strong>Fecha:</strong> {formatDate(detalle.report_date)}</p>
                <p><strong>Hora:</strong> {detalle.report_time}</p>
              </div>
              <div className="sm:col-span-2">
                <p><strong>Detalles del reporte:</strong> {detalle.report_details}</p>
                <p><strong>Detalles de partes:</strong> {detalle.report_part_details}</p>
              </div>
            </div>
            <div className="mt-6 flex gap-3 flex-wrap">
              <Button variant="primary" onClick={() => navigate(`/workshop-reports/edit/${detalle.id}`)}>
                Editar reporte
              </Button>
